...
public class OOPAssertions {
	...
	public static Object[] all(Object... elements){
		return elements;
	}
	/**
	 ...
	 */
	public static void assertArrayContainsAll(Object[] ary, Object[] elements, String msg) {
	...
	public static void assertInOrder(String string, String... names) {
		...
		for(int i=0; i<names.length; ++i) {
			...
			assertTrue(pos[i] >= 0, "Missing [" + n + "] in layout");
			
			if(i>0) {
				assertTrue(pos[i] > pos[i-1], "Element [" + n + "] should follow element [" + names[i-1] + "]");
			}
		}
	}
	
	public static void assertEndsWith(String string, String e, String msg) {
		String s = string.replaceAll("[ \\n\\t]+$", "");  // extra spaces at end of line
		assertTrue(s.endsWith(e),
				()->(msg==null?"":msg+": ") + "'" + (s.length()>e.length()?"..":"") + s.substring(Math.max(s.length()-e.length(),0)) + "' does not end with '" + e + "'");
	}
	...
	public static void assertSameIndent(String l, String... names) {
		...
		for(int i=0; i<names.length; ++i) {
			...
			assertTrue(pos >= 0, "Missing [" + n + "] in layout");
			indent[i] = pos - maxLe(starts,pos);
			if(prev>=0) {
				assertEquals(indent[i], prev,
						"Different indentation levels for [" + names[i-1] + "](" + indent[i-1] + ") and [" +
								n + "](" + indent[i] + ")");
			}
			prev = indent[i];
		}
	 ...
	 */
	static void assertContainTimes(int n, String sub, String s, String msg) {
		assertNotNull(s);
		int count = 0;
		for( int index = 0; (index=s.indexOf(sub,index)) > -1; ++index) {
			count++;
		}
		assertEquals(n, count, msg);
	}
	 ...
	 */
	public static void assertSameElement(Element expected, Element actual, String msg) {
		if( expected != actual) {
